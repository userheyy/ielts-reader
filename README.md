...
- **304 个听力 Part**，覆盖剑桥雅思 1–19，包含原题、答案、转写和时间轴。对话按完整发言切分，独白按句子切分。
- 剑雅 2 有 5 段源音频缺失或不完整，页面会明确提示；原文、题目和答案仍可查看。
...
- 听力精听（`listening.html`）：影子跟读、逐句跟读、变速 0.5–1.5×、单句循环、AB 复读、逐句听写判分、打点模式
...
以下资源只保存在本地。线上未配置时会显示相应提示，不会影响阅读和听力功能。
...
文章、题目和听力材料出自剑桥大学出版社《剑桥雅思官方真题集》，版权归原出版社所有；语法讲解框架参考旋元佑《英语魔法师之语法俱乐部》。本项目仅供个人学习使用，请勿用于商业用途或再次分发。API Key 和本地语法书提取内容不会提交到仓库。
